...
- F1 `orchestration.py` / `production_runtime.py`: retry/backoff + circuit breaker + dead-letter queue + anchor-batch idempotent retry + dead-letter JSONL persistence + persisted dead-letter redrive helper + semantic refresh policy mark 추가 (**IN_PROGRESS**)

## 2026-03-24 구현 감사 상태

- `pytest -q` 기준 전체 테스트: **81 passed**
- 코드/테스트 기준 사실상 완료 또는 강하게 반영된 영역
  ...
  - D1, D2, D3, D4
  - E1, E2, E3
  - F1 (batch retry/backoff/circuit breaker/DLQ + persisted dead-letter redrive helper + production runtime anchor re-entry + semantic refresh marking)
- 아직 todo 원문 기준으로 추가 구현이 필요한 영역
  ...
  - F1: semantic refresh의 실제 live semantic branch 연결 + 운영자 tooling/재처리 정책 확장

## 이번 변경 증적

- Test command: `python -m pytest -q tests/test_production_runtime.py tests/test_live_direct_vectorization.py`
- Test result: `11 passed`
- Regression command: `python -m pytest -q`
- Regression result: `81 passed`
- Rollback strategy:
  ...
  - `docs/runbooks/dead_letter_recovery.md`
